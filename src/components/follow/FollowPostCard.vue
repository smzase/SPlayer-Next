...
const postCardRef = shallowRef<HTMLElement | null>(null);
...
/** 收起长正文后平滑返回当前笔记卡片 */
const returnToPost = (): void => {
  void nextTick(() => {
    postCardRef.value?.scrollIntoView({
      behavior: "smooth",
      block: "start",
      inline: "nearest",
    });
...
<template>
  <div ref="postCardRef" class="scroll-mt-4">
      ...
      <article>
        ...
        >
          <ExpandableCommentContent
            v-if="post.text"
            :content-key="post.text"
            @collapse="returnToPost"
          >
            <FollowRichText :text="post.text" />
          </ExpandableCommentContent>
          ...
          <div
            ...
            :class="
              post.images.length === 1
                ? compactImages
                  ? 'h-[220px] w-[220px] max-w-full grid-cols-1'
