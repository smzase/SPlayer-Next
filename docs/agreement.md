...
2.2 本软件以**学习、技术研究与交流**为目的开源发布，不以营利为目的，**不接受任何商业合作、商业捐赠或商业授权**，亦不对任何商业用途提供保证或支持。
...
- 不得对本软件附加任何与 AGPL-3.0 相抵触的额外限制。
  ...
  3.5 当本协议与 AGPL-3.0 就开源、源码提供等事项存在冲突时，**以 AGPL-3.0 的相关约定为准**；本协议其余条款（如免责、责任限制、使用规范等）作为 AGPL-3.0 之外的附加使用条件继续有效。

---

## 四、授权范围与使用限制

4.1 在您遵守本协议及 AGPL-3.0 的前提下，开发者授予您一项非排他、可撤销、不可转让的许可，允许您在自有设备上安装与使用本软件，用于个人合法的音乐播放与管理。
...
- 将本软件、本软件的名称或标识用于任何商业销售、商业捆绑、付费分发或虚假宣传；
...
5.3.1 本软件提供**开放的插件扩展机制**（包括对 LX Music 格式插件的兼容）。该机制为**中立的技术接口**，**默认不指向任何特定服务，亦不内置、不推荐、不分发任何具体音源或插件**。

5.3.2 所有第三方插件均由**第三方社区独立开发与维护**，需由您主动从本地或网络导入后方可启用。开发者**无法审核、无法控制**第三方插件的行为、来源与所返回的内容，对其合法性、安全性、稳定性、准确性不作任何担保。
...
5.4.2 该等版权数据**仅可用于您个人的试听与学习研究**，不得用于任何商业或公开传播用途。**您应在产生该等版权数据后的 24 小时内将其从您的设备中清除。** 如您需要长期、完整地欣赏相关内容，请通过正版渠道购买、订阅并支持版权方。
...
6.2 上述凭据**仅用于您所请求的功能本身**，并**仅存储于您的本地设备**（其中流媒体服务器凭据通过操作系统提供的 `safeStorage` 加密保存）。开发者不会、也无技术能力将其上传至开发者控制的任何服务器。
...
9.1 本软件**不收集、不上传任何可识别您个人身份的信息（PII）**。为了提升软件稳定性与优化产品体验，本软件可能包含匿名的性能诊断与功能使用统计机制（如崩溃堆栈、系统版本等脱敏数据）。此类统计不包含任何个人身份信息，且**您可在软件设置中随时开启或关闭**。具体的个人信息与数据处理规则，请参阅随附的《隐私政策》。
...
10.1 **唯一官方渠道**：本软件仅通过 GitHub 官方仓库 <https://github.com/SPlayer-Dev/SPlayer-Next> 及官方网站 <https://splayer.imsyy.top> 发布。

10.2 **非官方分发声明**：任何第三方网站、网盘、应用商店、论坛或个人对本软件的转载、镜像、重新打包或再分发，均不受开发者控制、未经开发者验证，**不构成官方发布行为**。
